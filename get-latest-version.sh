...
wget http://mirror.bit.edu.cn/apache/hadoop/common/current/`curl -s http://mirror.bit.edu.cn/apache/hadoop/common/current/|sed -n 's/.*\(hadoop-.*.tar.gz\).*/\1/p'|grep -v src` -O hadoop.tar.gz
